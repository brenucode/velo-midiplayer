"""QWERTY playback engine (forked from modules/midiHandler/midiWindows.py).

Decoupled from the Tkinter UI: ``log`` and ``onFinished`` are injectable hooks
set by the Player. Config now comes from velo.backend.config (local, offline).
The note-simulation logic is unchanged from the original.
"""
...
onFinished = None  # Player sets this; called once when a (non-looping) song ends.
...
def playMidiFile(midiFile, startOffset=0.0):
    ...
    if not configuration.configData["midiPlayer"]["loopSong"] and not (stopEvent.is_set() or closeThread):
        if onFinished:
            onFinished()
