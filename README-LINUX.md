...
system's **WebKitGTK** with Web Audio for sound. The floating **mini-player**
and the **Select Music** window work too (see the note on desktop environments
below).
...
1. **The floating mini-player / Select Music want a compositor (and X11 is
   best).** They're drawn as always-on-top utility windows via GTK. A running
   compositor (the default on GNOME, KDE, and most modern desktops) lets them
   stay above your game without stealing its keyboard focus. On a bare X11
   session with no compositor, or on some Wayland compositors, "always on top"
   or "don't steal focus" may not be fully honoured — the windows still work,
   they just may not float over a fullscreen game as cleanly as on Windows.
2. **"Type the song into another app" needs X11.** Sending keystrokes into
...
3. **MIDI-output mode wants a synth.** To drive a real software synth via the
